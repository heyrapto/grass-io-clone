  ...
  return (
    <nav
      className={`fixed top-5 left-1/2 transform -translate-x-1/2 z-50 flex items-center justify-between w-[90%] transition-colors duration-300 rounded-[50px] min-h-[60px] py-[15px] px-[30px] ${
        scrolled ? "bg-white shadow-bottom border border-black" : "bg-transparent text-white"
      }`}
    >
        ...
        className={`min-w-[150px] text-[16px] border ${scrolled ? "border-none" : "border-white bg-transparent"}`}
        />
    </nav>
  )
}
